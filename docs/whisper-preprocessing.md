...
| 従来の処理 | 役割 |
| --- | --- |
| `highpass=f=120` | 低い周波数を減衰させる。風・振動などの低域成分が対象になり得るが、声の低域も変化する。 |
| `lowpass=f=8000` | 高い周波数を減衰させる。16 kHzへの変換とは別に適用していたフィルター。 |
| `dynaudnorm=f=200:g=7` | 200 msごとにピークを調べ、7フレームの窓でゲインを平滑化し、区間ごとの音量差をならす。小さな声と一緒に雑音も増幅し得る。 |
| `silenceremove` | 音量と継続時間の条件で区間を削除する。新しい経路ではSilero VADに置き換える。 |
| 16 kHz / mono / PCM s16le変換 | Whisperへ渡す形式の変換。すべての経路で維持する。 |
